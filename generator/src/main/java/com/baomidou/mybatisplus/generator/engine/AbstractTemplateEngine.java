...
public abstract class AbstractTemplateEngine {
    ...
    protected void outputStructMapper(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        String structMapperName = tableInfo.getStructMapperName();
        String structMapperPath = getPathInfo(OutputFile.structMapper);
        if (StringUtils.isNotBlank(structMapperName) && StringUtils.isNotBlank(structMapperPath)) {
            getTemplateFilePath(TemplateConfig::getStructMapper).ifPresent((structMapper) -> {
                String structMapperFile = String.format((structMapperPath + File.separator + "%s" + suffixJavaOrKt()), structMapperName);
                outputFile(new File(structMapperFile), objectMap, structMapper, getConfigBuilder().getStrategyConfig().structMapper().isFileOverride());
            });
        }
    }
    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            tableInfoList.forEach(tableInfo -> {
                ...
                outputStructMapper(tableInfo, objectMap);
    ...
    public Map<String, Object> getObjectMap(@NotNull ConfigBuilder config, @NotNull TableInfo tableInfo) {
        ...
        Map<String, Object> structMapperData = strategyConfig.structMapper().renderData(tableInfo);
        objectMap.putAll(structMapperData);
